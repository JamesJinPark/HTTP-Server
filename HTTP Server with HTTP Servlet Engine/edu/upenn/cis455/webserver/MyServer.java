...
// commented this out to integrate with MS2
...
//		} else {
	...
	public void run(String[] args, Socket socket, BufferedReader in, String line, ThreadPool threads) throws InterruptedException{//added for MS2
		...
		myRoutes.put(Route.of("GET", "405Error"), new HttpRequestHandler(){
			public void handle(HttpRequest request, HttpResponse response) {
	            String body = "405 Method Used Incorrectly\n\n" + request;
            	request.isError = true;
	            response.setBody(body);
		...
		myRoutes.put(Route.of("GET", "412Error"), new HttpRequestHandler(){
			public void handle(HttpRequest request, HttpResponse response) {
	            String body = "412 Precondition Failed\n\n" + request;
	            response.setBody(body);
				request.isError = true;
		...
		myRoutes.put(Route.of("GET", "501Error"), new HttpRequestHandler(){
			public void handle(HttpRequest request, HttpResponse response) {
	            String body = "501 Method Not Recognized\n\n" + request;
            	request.isError = true;
	            response.setBody(body);
		...
		httpServer.runServer(socket, in, line, threads);
		}
//	}
}
